...
#else
		/* Try multiple UTF-8 locale names for portability */
		static const char *utf8_locales[] = {
			"",              /* Use LANG/LC_ALL environment */
			"en_US.UTF-8",
			"en_US.utf8",
			"C.UTF-8",
			"UTF-8",
			"EN_US.UTF-8",   /* AIX style */
			NULL
		};
		int locale_set = 0;
		for (int i = 0; utf8_locales[i] != NULL; i++) {
			if (setlocale(LC_ALL, utf8_locales[i])) {
				locale_set = 1;
				break;
			}
		}
		if (!locale_set)
			I_Error("DG_Init: setlocale error - no UTF-8 locale available.\n"
				"Try: export LANG=en_US.UTF-8 or install UTF-8 locale support.");
#endif
	}

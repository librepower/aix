 ...
 *
 * Walks /proc/<pid>/stat (Linux) or /proc/<pid>/psinfo (AIX) to build process chain.
...
#ifdef _AIX
/* AIX doesn't have strcasestr, so we provide our own */
...
/*
 * Read /proc/<pid>/stat (Linux) or /proc/<pid>/psinfo (AIX) to get comm and ppid
 * Format (Linux): pid (comm) state ppid ...
 * Format (AIX): binary struct psinfo
 */
static int read_proc_stat(pid_t pid, char *comm, size_t comm_len, pid_t *ppid) {
#ifdef _AIX
    /* AIX: Read binary /proc/<pid>/psinfo */
    char path[64];
    struct psinfo psi;
    int fd;
    ...
    /* Extract process name - use pr_fname (basename) or pr_psargs (full command) */
    size_t len = strlen(psi.pr_fname);
    if (len >= comm_len) len = comm_len - 1;
    memcpy(comm, psi.pr_fname, len);
    comm[len] = '\0';
...
/*
 * Fallback: get PPID from /proc/<pid>/status (Linux) or /proc/<pid>/psinfo (AIX) if stat parsing fails
 */
static pid_t get_ppid_fallback(pid_t pid) {
    ...
    return psi.pr_ppid;
#else
    /* Linux: Read text /proc/<pid>/status */
    char path[64];
    char line[256];
